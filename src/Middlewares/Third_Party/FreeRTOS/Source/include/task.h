...
/* some version information was deleted so the code would be clean. keep next line as comment:
...
typedef enum
{
    ...
    eBlocked,
    /* The task being queried is in the Suspended state, or is in the
...
/* 几种通知动作： 无动作、设置bits、增加数值、允许覆盖的模式设置值、不允许覆盖
 * 的方式设置值 */
typedef enum
{
    ...
    eIncrement,
    /* Set the task's notification value to a specific value even if the previous
       value has not yet been read by the task. */
 ...
 */
/* 使用MPU的时候的内存区域数据结构，包括基地址、字节长度、参数 */
...
typedef struct xTASK_STATUS
{
    ...
    /* 任务号，这个应该有用才对，但是分析到现在这个参数似乎还没用上 */
    ...
    /* The priority to which the task will return if the task's current priority has
    ...
    /* The total run time allocated to the task so far, as defined by the run time
       stats clock.  See http://www.freertos.org/rtos-run-time-stats.html.  Only
    ...
    /* The minimum amount of stack space that has remained for the task since the task
       was created.  The closer this value is to zero the closer the task has come to
       overflowing its stack. */
    /* 用于指示堆栈的应用，数值越小越有可能出现堆栈溢出 */
...
typedef enum
{
    ...
    /* 标准的进入睡眠模式 */
    eStandardSleep,
    /* No tasks are waiting for a timeout so it is safe to enter a sleep mode that
       can only be exited by an external interrupt. */
    /* 没有任务等待超时，因此可以安全进入睡眠模式 */
    /* 这种睡眠模式只能够通过外部中断退出 */
